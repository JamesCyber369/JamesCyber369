# Hey, I'm James 👋

⚡ Electrician transitioning into Cybersecurity & IT Infrastructure

---

## 🙋 About Me

I'm an active electrician making a full career transition into Cybersecurity with a focus on Cloud Security and AI Security. Rather than just studying theory, I chose to get hands-on — acquiring real enterprise server hardware and building a fully functional home lab completely from scratch.

---

## 🎯 Current Focus

- 🔨 Building a 14-phase enterprise home lab from scratch
- 📚 Studying for CompTIA Security+
- 🔐 Targeting Cloud Security & AI Security roles
- 🖥️ Practicing on real Supermicro enterprise hardware

---

## 🛠️ Skills Being Built

### Hardware & Infrastructure
![Supermicro](https://img.shields.io/badge/Supermicro-Enterprise%20Hardware-red)
![Linux](https://img.shields.io/badge/Linux-Server%20Admin-yellow)
![IPMI](https://img.shields.io/badge/IPMI-Remote%20Management-blue)

### Virtualization
![Proxmox](https://img.shields.io/badge/Proxmox-Hypervisor-orange)
![Kubernetes](https://img.shields.io/badge/Kubernetes-K3s-blue)
![Docker](https://img.shields.io/badge/Docker-Containers-blue)

### Security Tools
![Wazuh](https://img.shields.io/badge/Wazuh-SIEM-green)
![Suricata](https://img.shields.io/badge/Suricata-IDS/IPS-blue)
![Nmap](https://img.shields.io/badge/Nmap-Network%20Scanning-blue)
![Cowrie](https://img.shields.io/badge/Cowrie-Honeypot-red)
![OPNsense](https://img.shields.io/badge/OPNsense-Firewall-darkblue)

### AI Security
![AI Security](https://img.shields.io/badge/AI%20Security-LLM%20Testing-purple)
![MLSecOps](https://img.shields.io/badge/MLSecOps-Pipeline%20Security-purple)

---

## 🚀 Active Project

### 🖥️ [Cybersecurity Home Lab](https://github.com/JamesCyber369/Cybersecurity-homelab)

Building a full enterprise home lab from 3 Supermicro servers acquired with no documentation. Physically identified all hardware, confirmed functionality, and building through 14 phases of cybersecurity infrastructure and tooling.

| Phase | Project | Status |
|---|---|---|
| [01](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/01-hardware-identification) | Hardware Identification & Inspection | ✅ Complete |
| [02](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/02-network-setup) | Network Setup & IPMI Access | ✅ Complete |
| [03](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/03-os-installation) | OS Installation — Proxmox VE | ✅ Complete |
| [04](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/04-proxmox-cluster) | Proxmox Configuration & First VM | ✅ Complete |
| [05](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/05-pfsense-firewall) | Firewall Deployment — OPNsense | 🔄 In Progress |
| [06](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/06-wazuh-siem) | SIEM Deployment — Wazuh | ✅ Complete |
| [07](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/07-kubernetes) | Kubernetes Cluster — K3s | ✅ Complete |
| [08](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/08-openstack-cloud) | Private Cloud — OpenStack | ⏭️ Skipped |
| [09](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/09-vulnerability-scanning) | Vulnerability Scanning — Nmap | ✅ Complete |
| [10](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/10-honeypot) | Honeypot Deployment — Cowrie | ✅ Complete |
| [11](https://github.com/JamesCyber369/Cybersecurity-homelab/tree/main/11-suricata-ids) | IDS/IPS — Suricata | ✅ Complete |
| 12 | Infrastructure as Code — Terraform/Ansible | 📋 Planned |
| 13 | AI/LLM Security Lab | 📋 Planned |
| 14 | MLSecOps Pipeline | 📋 Planned |

---

## 📜 Certifications

| Certification | Status |
|---|---|
| CompTIA Security+ | 🔄 In Progress |
| Certified AI Workforce Professional (CAIWP) | ✅ Complete |
| Fortinet NSE 1 & 2 | 🔄 In Progress |

---

## 📫 Connect

[![GitHub](https://img.shields.io/badge/GitHub-JamesCyber369-black)](https://github.com/JamesCyber369)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-James%20Sinclair-blue)](https://linkedin.com/in/james-sinclair-682286410)

---

*"The best way to learn is to build."* ⚡
